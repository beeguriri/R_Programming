...
# 실습: tbl_df() 함수 사용하기 
hflights_df <- tbl_df(hflights) #deprecated
hflights_df <- as_tibble(hflights) # Please use `tibble::as_tibble()` instead.
hflights_df
...
select(hflights_df, -(Year:DepTime)) # '-' 제외하고 출력

# 데이터셋에 칼럼 추가
# 실습: hflights_df에서 출발 지연시간과 도착 지연시간의 차이를 계산한 칼럼 추가하기
# 전체컬럼 출력되므로 화면이 부족해서 출력이 안됨...?
...
# mutate : 원래 데이터셋에 필드가 추가되는 것은 아님!
...
# 단계 2: 데이터프레임 합치기 
# df1 아랫쪽으로 df2 데이터 붙음
df_rows <- bind_rows(df1, df2)
df_rows


# 실습: 두 개의 데이터프레임을 열 단위로 합치기 
# 행의 개수가 다르면 warning message 및 데이터 손실
df_cols <- bind_cols(df1, df2)
df_cols


# 실습: 데이터프레임의 칼럼명 수정하기 
# rename(데이터프레임, 변경후칼럼명 = 변경전칼럼명)
...
#날짜별로 나열된 데이터(긴형식)
# ▼
# 고객번호 별 날짜로 buy 데이터를 sum (짧은형식)
# 실습: 넓은 형식(wide format)으로 변경하기 
wide <- dcast(data, Customer_ID ~ Date, sum)
wide
wide2 <- dcast(data, Date~Customer_ID, sum)
wide2
...
# id 두개로 변환
long <- melt(smiths, id = 1)
long <- melt(smiths, id = 1:2)
long

# 단계 3: 긴 형식을 넓은형식으로 변경하기 
wide3 <- dcast(long, subject + time ~ ...)
wide3
