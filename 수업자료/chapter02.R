...
# 음수값 인덱스 빼고 출력함
v1[-1]; #v1 : [-5 20 21 22 23 12 -2 -1  0  1  2  3]
v1[-c(2, 4)]; #v1 : [13 20 22 23 12 -2 -1  0  1  2  3]
v1[-c(2:4)]; 
v1[-c(2, 5:10, 1)];
...
vec <- c(1:12)
vec
arr <- array(vec, c(3, 2, 2)) #row,col,dimension [3][2][2]
arr
...
name <- c("hong", "lee", "kim")
pay <- c(150, 250, 300)
vemp <- data.frame(No = no, Name = name, Pay = pay) #필드의 값으로 데이터 만듦
vemp
...
apply(df[ , c(1, 2)], 1, sum) #행기준 더하기
apply(df[ , c(1, 2)], 2, sum) #열기준 더하기
df

# 실습: 데이터프레임의 부분 객체 만들기 
x1 <- subset(df, x >= 3) #부분집합
x1
...
str_extract_all(string, "[a-z]{3}") #len : 3
str_extract_all(string, "[a-z]{4,}") #len : 3~
str_extract_all(string, "[a-z]{3,5}") #len : 3~5
