...
# 단계 2: 오라클 데이터베이스 연결
conn <- dbConnect(drv,
                  "jdbc:oracle:thin:@//127.0.0.1:1521/xe", "c##kang", "1234")
...
# 단계 3: 레코드 삽입(insert)
query = "insert into student values('s7', 'park', 'busan', '25')"
dbSendUpdate(conn, query)

# 단계 4: 조건 검색 - 나이가 25세 이상인 레코드 조회 
query = "select * from student where age >= 25"
result <- dbGetQuery(conn, query)
result

# 단계 5: 레코드 수정 - sno=s2인 데이터의 age를 40으로 수정
query = "update student set age = 40 where sno = 's2'"
dbSendUpdate(conn, query)

# 수정된 레코드 조회
query = "select * from test_table where sno = 's2'"
dbGetQuery(conn, query)

# 단계 6: 레코드 삭제 - sno=s2인 레코드 삭제
query = "delete from student where sno = 's3'"
dbSendUpdate(conn, query)
...
#dbWriteTable(conn, "goods", recode) #이미 있는 테이블에는 저장 불가
