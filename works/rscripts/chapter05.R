...
barplot(chart_data, xlim = c(0, 600), horiz = T, 
        ylab = "매출액(단위: 만원)",
        xlab = "년도별 분기 현황", 
        col = rainbow(8), space = 0.5, cex.names = 0.5,
        main = "2018년도 vs 2019년도 매출현항 비교")
...
# 단계 1: 기본 산점도 시각화 
# runif : uniform분포를 따르는 random 함수
...
# 단계 3: 산점도 시각화 
plot(x, y)
plot(x1, y1, xlim=c(1,5), ylim = c(1,5))
# 단계 4: 교차 테이블로 데이터프레임 생성
xy.df <- as.data.frame(table(x, y))
xy.df <- as.data.frame(table(x1, y1))
xy.df
...
plot(x1, y1, 
     pch = "☆", col = "blue", cex = 2 * xy.df$Freq,
     xlab = "x 벡터의 원소", ylab = "y 벡터 원소",xlim=c(1,5), ylim = c(1,5))
