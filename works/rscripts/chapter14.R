...
# Factor2와 Factor3 시각화
plot(result$scores[ , c(2, 3)],
     main = "Factor2과 Factor3 요인점수 행렬")
text(result$scores[ , 2], result$scores[ , 3],
     labels = name, cex = 0.7, pos = 3, col = "blue")

# 단계 4: 요인적재량 추가 
points(result$loadings[ , c(2, 3)], pch = 19, col = "red")
text(result$loadings[ , 2], result$loadings[ , 3],
     labels = rownames(result$loadings), 
     cex = 0.8, pos= 3, col = "red")
...
data.spss <- as.data.set(spss.system.file('./data/drinking_water.sav'), fileEncoding = "euc-kr")
data.spss[1:11]
...
result2
# factor 수를 증가시켜봐도 설명력이 부족함을 확인할 수 있음
...
# Cumulative Var 0.675인데.. 이것도 유효하지 않은것아닌가...?;ㅁ;
result3 <- factanal(dw_df, factor = 3, rotation = "varimax")
result3 

# 단계 1(2): q5를 제외하고 데이터프레임 생성
dw_df2 <- drinking_water_df[-5]
en4 <- eigen(cor(dw_df))
en4
result5 <- factanal(dw_df2, factor = 3, rotation = "varimax")
result5 
...
s1 <- data.frame(dw_df2$Q8, dw_df2$Q9, dw_df2$Q10, dw_df2$Q11)
c1 <- data.frame(dw_df2$Q1, dw_df2$Q2, dw_df2$Q3)
p1 <- data.frame(dw_df2$Q4, dw_df2$Q6, dw_df2$Q7)
...
satisfaction1 <- round(
  (s1$dw_df.Q8 + s1$dw_df.Q9 + s1$dw_df.Q10 + s1$dw_df.Q11) / ncol(s), 2)
closeness1 <- round(
  (c1$dw_df.Q1 + c1$dw_df.Q2 + c1$dw_df.Q3) / ncol(s), 2)
pertinence1 <- round(
  (p1$dw_df.Q4 + p1$dw_df.Q6 + p1$dw_df.Q7) / ncol(s), 2)
...
result4 <- factanal(dw_df, factor = 3, rotation = "varimax")
result4

drinking_water_factor_df1 <- data.frame(satisfaction, closeness, pertinence)
colnames(drinking_water_factor_df1) <- c("제품만족도", "제품친밀도", "제품적절성")
cor(drinking_water_factor_df1)
...
sd(product$제품_적절성); 
sd(product$제품_만족도)
cor(product) # default : method = "pearson"
